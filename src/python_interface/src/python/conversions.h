...
std::map<std::string, T> fromDict(const boost::python::dict& values)
{
	...
	// values.iteritems() => values.items() for AttributeError: 'dict' object has no attribute 'iteritems'
	// https://stackoverflow.com/questions/30418481/error-dict-object-has-no-attribute-iteritems
	for (boost::python::stl_input_iterator<boost::python::tuple> it(values.items()), end; it != end; ++it) {
